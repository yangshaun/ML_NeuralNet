...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        backward_output = np.multiply(grad,(np.ones(X.shape) - np.power(np.tanh(X),2)))
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    # print (module_name + '_' + key) 
                    
                    # print (momentum[module_name + '_' + key])
                    # print (_alpha * momentum[module_name + '_' + key])
